...
RUN apt-get update \
    ...
    && apt-get install -y gnupg gosu curl ca-certificates zip unzip git supervisor sqlite3 libcap2-bin libpng-dev python2 dnsutils librsvg2-bin fswatch iputils-ping \
...
ENV PHP_IDE_CONFIG="serverName=<< project_name >>_server"
